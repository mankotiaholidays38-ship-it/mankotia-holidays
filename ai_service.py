...
def generate_ai_itinerary(destination: str, days: int = 4, budget: str = "Standard", travel_style: str = "Family", travelers: str = "2 Adults", special_requests: str = "", pickup_location: Optional[str] = None, drop_location: Optional[str] = None) -> dict:
    ...
    return {
        ...
        "estimated_cost_inr": f"₹{days * 3200:,} - ₹{days * 5800:,} per person",
...
CONCIERGE_TOPICS = [
    (["golden triangle", "delhi agra jaipur"], f"🏛️ **Golden Triangle Specials:** We offer Golden Triangle Classic (6D/5N) and Delhi-Mathura-Agra (4D/3N) with Taj Mahal, Agra Fort, Qutub Minar, and Amber Fort from ₹9,999/person. Connect on WhatsApp (+{AGENCY_WHATSAPP}) or call {AGENCY_PHONE}!"),
    (["agra", "taj mahal", "fatehpur sikri"], f"🕌 **Agra Taj Express:** Same Day Agra Taj Express (₹4,499) & Mughal Marvels 2D/1N (₹6,999) with Taj Mahal sunrise, Agra Fort, Mehtab Bagh, and approved guide. WhatsApp (+{AGENCY_WHATSAPP}) or call {AGENCY_PHONE}!"),
    (["delhi", "akshardham", "qutub minar"], f"🏛️ **Delhi Capital Sightseeing:** Same Day Delhi Express (₹3,499) and 2D/1N Capital Heritage (₹6,499) covering Red Fort, Chandni Chowk rickshaw ride, Qutub Minar, and Akshardham. WhatsApp (+{AGENCY_WHATSAPP})!"),
    (["mathura", "vrindavan", "banke bihari", "prem mandir"], f"🦚 **Sacred Mathura & Vrindavan:** Same Day Braj Darshan (₹3,999) and 3D/2N Complete Braj Dham (₹8,499/person) covering Krishna Janmasthan, Banke Bihari Ji, Prem Mandir Light Show, and Gokul. Call {AGENCY_PHONE}!"),
    (["jaipur", "amber fort", "chokhi dhani"], f"👑 **Royal Jaipur Tours:** Same Day Jaipur (₹4,999), Weekend Heritage 2D/1N (₹6,999), and Grand Forts 3D/2N (₹9,999) with Amber Fort Jeep ascent and Chokhi Dhani dinner. Call {AGENCY_PHONE}!"),
    (["char dham", "chardham", "yamunotri", "gangotri"], f"🕉️ **Char Dham Yatra 2026:** All-inclusive 10N/11D package covering Yamunotri, Gangotri, Kedarnath, and Badrinath with pickup/drop from Haridwar/Dehradun starting at ₹34,999/person. WhatsApp (+{AGENCY_WHATSAPP}) or call {AGENCY_PHONE}!"),
    (["do dham", "dodham", "kedar badri"], f"🙏 **Do Dham Yatra (Kedarnath & Badrinath Ji):** Popular 5N/6D spiritual circuit from ₹22,500/person with optional Helicopter shuttle at Phata/Sirsi. Message on WhatsApp (+{AGENCY_WHATSAPP})!"),
    ...
    (["uttarakhand", "nainital", "mussoorie", "corbett", "auli"], f"🏔️ **Uttarakhand Specials:** Nainital Lakes, Corbett Safari, Mussoorie, Auli Skiing, and Chopta Trek from ₹15,999/person. WhatsApp (+{AGENCY_WHATSAPP}) for family offers!"),
    (["himachal", "manali", "shimla"], f"🏔️ **Himachal Escapes:** Packages covering Manali, Solang Valley, Atal Tunnel, Rohtang Pass, and Shimla starting from ₹12,999/person. Connect on WhatsApp (+{AGENCY_WHATSAPP})!"),
    (["kashmir", "gulmarg", "pahalgam", "srinagar"], f"🌸 **Kashmir Heaven on Earth:** Airport transfers, Dal Lake houseboats, Gulmarg Gondola rides, and Pahalgam from ₹21,999/person. Call {AGENCY_PHONE} or WhatsApp (+{AGENCY_WHATSAPP})."),
